...
- Created `frsca-ml/schemas/in-toto/ml-training-predicate.json` (Draft-07
  schema).
- Created `frsca-ml/schemas/in-toto/dataset-provenance.json` (Draft-07
  schema).
- Created `frsca-ml/schemas/cyclone-dx/ai-bom-1.6-template.json` (CycloneDX
  1.6 example).
...
- Updated `model-training-task.yaml` and `ml-supply-chain-pipeline.yaml` to
  support `context-dir`.
